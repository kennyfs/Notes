...
Plot assets and possible combinations of asset on a Stdev-Expected return graph.  
Let S=(19,10), B=(8,5).  
Stock is at upper right of Bond, so the combination looks like a 不是哥們 curve. The curve will pass S and B, and  
* $\rho_{SB}\approx 0$: 正常情況，「不是哥們」曲線  
* $\rho_{SB}=-1$: 極端情況，三角形(S打到y軸後反射到B)  
* $\rho_{SB}=1$: 極端情況，SB直線  

### with risk-free assets  
Connect a line between the point $(0,r_f)$ to points on the 不是哥們 curve, the slope is the Sharpe Ratio $\frac{r-r_f}{\sigma}$. So we want to find the point of tangency as it gives the highest Sharpe Ratio.  
Next, we can choose a point on the line, which represents different ratio of risky-nonrisky assets.  

### Efficient Frontier  

Given multiple assets, we can find a curve which is convex to upper left.  
...
Suppose for contradiction that $\alpha_P>0$. We can build a long-short strategy, long P and short a mimicking portfolio($\beta_P$ on market index, $1-\beta_P$ on T-bills(risk-free)).  
...
如果倒掛(負斜律)就代表預期未來會大幅降息，可能是因為經濟不景氣，可能會出事。  

### Liquidity Preference Theory  

長期債券有流動性風險，因此需要更高的殖利率。  